...
Run `uv run main.py`  oder `python main.py`
...
``` 
Müssen mehrere Bücher ausgetauscht werden wird eine Liste mit Dictionaries verwendet.
...
Aktuell befindet sich die Logik in der `main.py` Datei. Die Funktionen zur Verwaltung sollen später in `management.py` integriert werden.
...
- Füllen Sie die Schnittstellendokumentation aus.

## Part 3: Vervollständigen
- Vervollständigen Sie das Programm indem Sie die Funktionen zur Logik des Programms in der `management.py` ausprogrammieren und in die `main.py` integrieren.
